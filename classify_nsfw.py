...
def benchmark(model, transformer, iters=1000):
    ...
    image_data = [(img, open("images/" + img).read()) for img in images]
...
def main(argv):
    ...
    if args.input_file:
        ...
        image_data = open(img_path).read()
        classify((img_path, image_data), model, transformer)
    elif args.benchmark:
        model, transformer = load_model()
        benchmark(model, transformer, int(args.benchmark))
    else:
        parser.print_help()
        print "nothing to do"
